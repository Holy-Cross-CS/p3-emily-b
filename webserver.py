...
class Topic:
    ...
    def add_like(self):
        self.like_count += 1  # Increment like count
        self.version += 1  # Increment version

def move_topic_to_end(topics, topic_name):
    if topic_name in topics:
        topic = topics.pop(topic_name)  # Remove the topic from the dictionary
        topics[topic_name] = topic  # Re-add the topic at the end
...
def update_topic_list_version():
    global topic_list_version_number
    with updates:  
        topic_list_version_number += 1
        updates.notify_all() 

# Function to initialize default topics
def initialize_default_topics():
    global topics
    topics = {
        "whatever": Topic("whatever"),
        "holycross": Topic("holycross"),
        "music": Topic("music")
    }
...
# handle_http_get_whoami() returns a response for GET /hello

def handle_http_get_whoami(req,conn):
    log("handling http get whoami request")

    ipAddress, clientPort = client_addr
    userAgent = get_header_value(req.headers, "User-Agent")
    preferredLang = get_header_value(req.headers, "Accept-Language").split(',', 1)[0].strip()
    ...
    <html>
        ...
        <body>
            ...
            <p> Port of client: {clientPort} </p>
            <p> "user-agent": {userAgent}</p>
            <p> Preferred language: {preferredLang}</p> 
            <p> Cookie values: {cookies} </p>
            <p> the value of the "DNT" (do-not-track) header: {dnt} </p>
            <p> "Sec-GPC" (Global Privacy Control) header: {sec_gpc} </p>
        </body>
    </html>
    """
...
def handle_http_get_hello(req, conn):
    ...
    # Undoing URL quoting
    s = urllib.parse.unquote(req.path)
    ...
    if req.path == "/hello":
        name = "World"
        colors = [ "coral", "crimson", "navy", "purple", "tomato", "teal", "wheat" , "red", "orange", "yellow", "green", "blue"
        , "purple", "pink", "white", "black", "brown", "maroon" ]
    
        favcolor = random.choice(colors)
    elif req.path.startswith("/hello?username="):
        # Split the URL at the '?' to separate the path and query string
        _, query_string = s.split('?', 1)

        # Split the query string into individual parameters
        parameters = query_string.split('&')

        # Iterate through each parameter to parse out username and color
        for parameter in parameters:
            key, value = parameter.split('=', 1)
            if key == 'username':
                # Replace '+' with spaces for the username
                name = value.replace('+', ' ')
            elif key == 'favorite':
                favcolor = value.replace('+', ' ')
    ...
    # Get a random color based on hexadecimal format
    def get_random_hex_color():
        return "#{:06x}".format(random.randint(0, 0xFFFFFF))
    
    random_color = get_random_hex_color()
    ...
    <body>
        ...
        <p> The current date is: <strong>{current_date}</strong> </p> 

        <p> You have visited this page an unknown number of times before. </p>

        <p> Maybe your favorite color is... {favcolor}?? </p>

        <p> Hit page refresh (F5) to refresh this page and see the contents MOSTLY STAY THE SAME?? </p>

        <p> You can also go to these exciting pages: </p>
        <ul>
            <li> <a href="/hello">/hello - this page</a> </li>
            <li> <a href="/status">/status - status and statistics</a> </li>
            <li> <a href="/quote">/quote - a random quote</a> </li>
            <li> <a href="/welcome.html">/welcome.html - a welcome page</a> </li>
            <li> <a href="/index.html">/index.html - some fancy web page</a> </li>
        </ul>
        <br> Use this form to change your name and color!
        <form method="GET" action="/hello"
            style="background: gray; padding: 5px; margin: 5px; display: inline-block;">
            Enter your name: <input type="text" name="username" value=""> <br>
            Favorite color: <input type="text" name="favorite" value=""> <br>
        <input type="submit" value="Greet Me!"></form></li>
    </body>
    </html>
    """
    resp = Response("200 OK", "text/html", msg)
...
# get whisper
def handle_http_get_whisper(req,conn):
    global topic_list_version_number
    version_str = req.path.split("version=")[1]
    version = int(version_str)
    with updates:  # Use the condition object for synchronization
        while topic_list_version_number < version:
            updates.wait()
        log(f"Handling http get whisper request with version {version} ") 
        msg = str(topic_list_version_number) + "\n"

        # sort topics by most recently added tag
        for topic_name, topic_data in reversed(list(topics.items())):
            message_count = str(topic_data.message_count)
            like_count = str(topic_data.like_count)
            msg += message_count + " " + like_count + " " + topic_name + "\n"  
    return Response("200 Ok", "text/plain" , msg)

# get whisper feed for a topic
def handle_http_get_whisperfeed(req,conn):
    global topics
    global topic_list_version_number
    try:
        version_str = req.path.split("version=")[1]
        ...
        return Response("400 BAD REQUEST", "text/plain", "Malformed or missing version parameter")

    with updates:
        while topic_list_version_number < version:
            updates.wait()
        try:
            path_and_query = req.path.split("?")
            path = path_and_query[0]
            path_parts = path.split("/")
            topic1 = path_parts[3]
        except IndexError:
            return Response("400 BAD REQUEST", "text/plain", "Malformed URL")

        log(f"Handling http get whisper feed request for topic {topic1} and version {version}")
        if topic1 in topics:
            msg = f"{version}\n"
            messages = topics[topic1].messages
            if messages:
                # Get the 10 most recent messages
                most_recent_messages = messages[-10:]
                # Reverse to show the newest message at the top
                most_recent_messages = most_recent_messages[::-1]
                
                # Join the messages and append to the response
                msg += "\n".join(f"- {message}" for message in most_recent_messages) + "\n"
            else:
                msg += ""  # No messages to show
        else:
            return Response("404 ERROR", "text/plain", "TOPIC NOT FOUND")
    return Response("200 OK", "text/plain", msg)

# Posting messages
def handle_http_post_messages(req,conn):
    # Ensure proper locking for accessing the shared topics data
    global topics
    
    # Parse body
    lines = req.body.split("\n")
        
    # Expecting exactly two lines: one for tags, one for message
    #if len(lines) != 2:
     #   return Response("400 BAD REQUEST", "text/plain", "More than two lines!")

    tags_line = lines[0]
    message_line = lines[1]

    # Check if tags and message follow the expected format
    if not tags_line.startswith("tags... ") or not message_line.startswith("message... "):
        return Response("400 BAD REQUEST", "text/plain", "BAD FORMATTING!")

     # Extract topics and message
    topics_list = tags_line[len("tags... "):].split()  # Extract topics
    message = message_line[len("message... "):].strip()  # Extract message
    if not message:  # Handle empty message
        return Response("200 OK", "text/plain", "EMPTY MESSAGE: IGNORE")

    if not topics_list:  # Handle case with no topics
        topics_list = ["whatever"]  # Default to #whatever if no topics are provided

    with updates:  # Ensure the topics dictionary is modified safely
        for topic_name in topics_list:
            if topic_name not in topics:
                topics[topic_name] = Topic(topic_name)

            topics[topic_name].add_message(message)
            move_topic_to_end(topics, topic_name)

        update_topic_list_version()  # Increment version and notify waiting clients
...
def handle_http_post_likes(req,conn):
    ...
    try:
        ...
        return Response("400 ERROR", "text/plain", "Malformed url")

    if topic1 in topics:
        with updates:
            topics[topic1].like_count += 1 # Increment likes
            topic_list_version_number += 1  # Increment the version for the topics list
            move_topic_to_end(topics, topic1)
            updates.notify_all()
    else:
        return Response("404 ERROR", "text/plain", "No topic with that name")
...
def handle_http_post(req,conn):
    if req.path == "/whisper/messages":
        resp = handle_http_post_messages(req,conn)
    elif req.path[0:13] == "/whisper/like":
        resp = handle_http_post_likes(req,conn)
    else:
        return Response("403 FORBIDDEN", "text/plain", "Permission denied: " + req.path)
    return resp
...
def handle_http_get_file(url_path):
    ...
    if not os.path.isfile(file_path):
        ...
        if "index.html" in file_path:
            ...
            <header>
                <meta charset="UTF-8">
                <meta name="viewport" content="width=device-width, initial-scale=1.0">
                <title> Couldn't find index.html </title>
            </header>
            <body>
                <h1> There is no "index.html file" </h1>
                Here are a list of links you can click.
                <ul>
                    <li> <a href="http://{server_host}:{server_port}/">http://{server_host}:{server_port}/</a> </li>
                    <li> <a href="http://{server_host}:{server_port}/welcome.html">http://{server_host}:{server_port}/welcome.html</a> </li>
                    <li> <a href="http://{server_host}:{server_port}/status">http://{server_host}:{server_port}/status</a> </li>
                </ul>
            </body>
            </html>
            """
            return Response("That file does not exist", "text/html", msg)
        else:
    ...
    try:
        ...
        # Determine mime_type
        if extension.lower() == "jpeg" or extension == "jpg":
            mime_type = "image/jpeg"
        elif extension.lower() == 'png':
            mime_type = "image/png"
        elif extension.lower() == "html" or extension.lower() == "htm":
            mime_type = "text/html"
...
def handle_http_get(req, conn):
    ...
    elif req.path[0:16] == "/hello?username=":
        resp = handle_http_get_hello(req,conn)
    elif req.path == "/quote":
        resp = handle_http_get_quote()
    elif req.path == "/whoami":
        resp = handle_http_get_whoami(req,conn)
    elif req.path[0:24] == "/whisper/topics?version=":
        resp = handle_http_get_whisper(req,conn)
    elif req.path[0:14] == "/whisper/feed/":
        resp = handle_http_get_whisperfeed(req,conn)
    ...
    # Check if the request is for a directory
        ...
        # Serve index.html if the path ends with '/'
        resp = handle_http_get_file(req.path + "index.html")

    else:
        # Handle directories without '/' (e.g., "/subdir" should serve "/subdir/index.html")
